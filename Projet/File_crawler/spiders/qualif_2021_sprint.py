...
class F1Spider(scrapy.Spider):
    ...
    # def parse_gp():
    #   permet de donner le lien de chaque Grand Prix ont une session Sprint     
    #   
    ...
    def parse_gp(self, response):
        ...
        for num in li:
            ...
            if num == 12 : 
                ...
                for link in all_links.values():
                    yield Request(link, callback=self.parse_qualif_spa)
            else:
                continue
    ...
    def parse_qualif(self, response):
        all_links = {
            name:response.urljoin(url) for name, url in zip(
            response.css(".resultsarchive-side-nav").css(".side-nav-item")[8].css("a::text").extract(),
            response.css(".resultsarchive-side-nav").css(".side-nav-item")[8].css("a::attr(href)").extract())
        }
        for link in all_links.values():
            yield Request(link, callback=self.parse_category)
    
    # def parse_qualif():
    #   va dans la liste et va chercher l'onglet qualif Spa car exception
