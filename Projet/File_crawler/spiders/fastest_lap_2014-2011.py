...
class F1Spider(scrapy.Spider):
    ...
    def parse(self, response):
        all_links = {
            ...
            response.css(".resultsarchive-filter-container").css(".resultsarchive-filter-wrap")[0].css(".resultsarchive-filter-item")[9:11].css("span::text").extract(),
            response.css(".resultsarchive-filter-container").css(".resultsarchive-filter-wrap")[0].css(".resultsarchive-filter-item")[9:11].css("a::attr(href)").extract())
        }
    ...
    # def parse_fastest():
    #   va dans la liste et va chercher l'onglet fastest
    ...
    def parse_category(self, response):
        ...
        for article in response.css(".resultsarchive-table").css("tbody").css("tr"):
            ...
            Avg_Speed = article.css(".dark.bold")[2].css("td::text").extract_first()
            yield ArticleItem(
                title = title,
                Date = Date,
                Position = Position,
